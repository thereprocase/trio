...
class Spool:
    def __init__(self, path, binding):
        ...
        identity = {k: binding[k] for k in ('endpoint', 'thread_id', 'url', 'channel', 'member_id', 'filter')}
    ...
    def deliver(self, client, thread_id):
        ...
        for mid, payload in self.db.execute("SELECT message_id,payload FROM events WHERE state='pending' ORDER BY message_id").fetchall():
            ...
            try:
                response = client.request('turn/start', {'threadId': thread_id, 'input': [],
                    'toolOutput': {'name': 'quartet_event', 'namespace': None, 'output': payload}})
    ...
    try:
        ...
        while True:
            ...
            if not isinstance(poll, dict) or poll.get('error'):
                raise RuntimeError('Quartet refused the poll; check channel membership and session binding')
