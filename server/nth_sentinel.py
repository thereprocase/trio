"""
Unified monitoring sentinel for nth agents (v5).

Replaces both nth_wait.py (message detection) and
nth_watchdog.py (heartbeat/cadence monitoring) with a single
...
Usage:
    python nth_sentinel.py <channel> <member_id> [options]

Options:
    --max-runtime SECONDS       (default: 3540 — 59 min, from nth_constants.py)
...
from nth_constants import SLEEPING_KEYWORDS, MAX_RUNTIME_S
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(json.dumps({
            "error": "Usage: nth_sentinel.py <channel> <member_id> [options]"
        }))
        sys.exit(1)
