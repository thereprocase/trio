"""
Background polling script for nth (legacy, deprecated).
...
of nth_poll (auto-advance) or nth_ack (explicit). This eliminates
the watermark race condition where nth_wait and nth_poll both
...
Usage:
    python nth_wait.py <channel> <member_id> [--timeout SECONDS]
        ...
        try:
            ...
            try:
                ...
                if "no such table" in str(e):
                    return {"error": "Database not initialized. Run nth_connect first to create the schema."}
                raise
            ...
            if unread:
                ...
                # nth_poll or nth_ack will advance the DB watermark when
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(json.dumps({"error": "Usage: nth_wait.py <channel> <member_id> [--timeout SECONDS]"}))
        sys.exit(1)
