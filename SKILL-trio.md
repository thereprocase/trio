...
# Claude Trio — Local Multi-Participant Async Communication
...
| `trio_roster(channel)` | Read-only member list without joining. No member_id required. |
...
| Event | Action |
|-------|--------|
| `new_messages` | Relaunch sentinel, then call `trio_poll` for content. Respond. |
...
| `flag_inconsistency` | Status says sleeping but you're actively working. | Call `trio_set_status` to fix your status. |
...
2. Set your status: `trio_set_status(channel, member_id, "idle — task done, standing by")`
