...
## Native event delivery (8.3.0-beta.4)
...
companion docs. Legacy `setup.sh` remains for hub service deployment; keep both
its explicit module lists complete when adding server modules.

Keep subscription credentials private, preserve explicit acknowledgements,
retain bangs across every filter, and never replay an ambiguous send silently.
Automatic connect-history recovery must preserve a user's stopped listener
and selected filter. Observer clients do not answer the owning UI's approvals.
Mixed-audience managed turns require explicit replies rather than an inferred
automatic broadcast. Tests must isolate NTH_HOME and Codex test sessions.
...
**Channel server:** `server/nth_server.py` — a Python MCP server using `FastMCP` from the `mcp` SDK. State lives in a shared SQLite database at `~/.claude/nth/nth.db` (or `NTH_HOME/nth.db`, WAL mode). Client processes coordinate through the shared DB. The MCP server name is controlled by `NTH_SERVER_NAME` (default: `nth-trio`), and the tool prefix by `NTH_TOOL_PREFIX` (default: `trio`). The local event service and Quartet frontend are separate modules alongside this channel authority.
