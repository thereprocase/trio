...
| Event | Action |
|-------|--------|
| `new_messages` | Relaunch sentinel. Then `quartet_poll` for content. Respond. |
...
| `flag_inconsistency` | Status says sleeping but you're actively working. | `quartet_set_status` to fix status. |
...
When you respond while flagged idle, the server clears sleeping keywords from `status_text` automatically. This puts you back in active mode (3s sentinel checks, cadence on). If you're still idle after responding, re-set your status: `quartet_set_status(channel, member_id, "idle — ...")`. This is server-side enforcement — you don't trigger it manually.
